...
namespace WebApp.Controllers
{
    ...
    public class FasilitasController : Controller
    {
        ...
        private string _path_controler = "/Fasilitas/";
        private string _path_view = "/Views/Fasilitas/Fasilitas/";
        private readonly string _table_name = "ta_fas";
        private readonly string _table_title = "Fasilitas";

		private static IHostingEnvironment _hostingEnvironment;
        public FasilitasController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(FasilitasModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = FasilitasModel._pkKey;
                    ViewData["displayItem"] = FasilitasModel.GetDisplayItem();
                    ViewData["column_att"] = FasilitasModel.GetGridColumn();
                    ViewData["param"] = FasilitasModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(FasilitasModel.ActionRequest param)
        {
            GridData data = FasilitasModel.GetListData(HttpContext, param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    FasilitasModel.ViewModel fieldModel = new FasilitasModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = FasilitasModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.jenis_fasilitas_id = dr["jenis_fasilitas_id"].ToString();
                                fieldModel.jenis_fasilitas_id_text = dr["jenis_fasilitas_id_text"].ToString();
                                fieldModel.jumlah_fasilitas = dr["jumlah_fasilitas"].ToString();
                    ...
                    ViewData["fieldModel"] = fieldModel;
                    ViewData["fasilitas_id"] = fieldModel.id;
                    ViewData["psa_id"] = fieldModel.psa_id;
                    ViewData["jenis_fasilitas_id"] = fieldModel.jenis_fasilitas_id;
                    ViewData["status_kepemilikan_id"] = fieldModel.status_kepemilikan_id;
                    ViewData["AllowView"] = SecurityHelper.HasRule(HttpContext, _rule_view) ? "1" : "0";
                    ViewData["AllowAdd"] = SecurityHelper.HasRule(HttpContext, _rule_add) ? "1" : "0";
                    ViewData["AllowEdit"] = SecurityHelper.HasRule(HttpContext, _rule_edit) ? "1" : "0";
                    ViewData["AllowDelete"] = SecurityHelper.HasRule(HttpContext, _rule_delete) ? "1" : "0";
        ...
        [HttpGet]
        public IActionResult Add(FasilitasModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //FasilitasModel.ViewModel fieldModel = new FasilitasModel.ViewModel();
                    fieldModel.id = FasilitasModel.GetNewId().ToString();
                    fieldModel.jumlah_fasilitas = "0";
                    ViewData["fieldModel"] = fieldModel;
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = FasilitasModel.GetViewData(PkValue);
                    FasilitasModel.ViewModel fieldModel = new FasilitasModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = FasilitasModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(FasilitasModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = FasilitasModel.GetNewId().ToString();
                    ...
                    data["jenis_fasilitas_id"] = fieldModel.jenis_fasilitas_id;
                    data["jumlah_fasilitas"] = fieldModel.jumlah_fasilitas;
                    ...
                    result = FasilitasModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(FasilitasModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(FasilitasModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(FasilitasModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = FasilitasModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = FasilitasModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = FasilitasModel.LookupData(param);
            return Json(data);
        }
