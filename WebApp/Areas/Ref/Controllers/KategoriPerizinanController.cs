...
namespace WebApp.Areas.Ref.Controllers
{
    ...
    [Area("Ref")]
    public class KategoriPerizinanController : Controller
    {
        ...
        private string _path_controler = "/Ref/KategoriPerizinan/";
        private string _path_view = "/Areas/Ref/Views/KategoriPerizinan/";
        private readonly string _table_name = "ref_kategori_perizinan";
        private readonly string _table_title = "Kategori Perizinan";

		private static IHostingEnvironment _hostingEnvironment;
        public KategoriPerizinanController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(KategoriPerizinanModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = KategoriPerizinanModel._pkKey;
                    ViewData["displayItem"] = KategoriPerizinanModel.GetDisplayItem();
                    ViewData["column_att"] = KategoriPerizinanModel.GetGridColumn();
                    ViewData["param"] = KategoriPerizinanModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(KategoriPerizinanModel.ActionRequest param)
        {
            GridData data = KategoriPerizinanModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    KategoriPerizinanModel.ViewModel fieldModel = new KategoriPerizinanModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = KategoriPerizinanModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(KategoriPerizinanModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //KategoriPerizinanModel.ViewModel fieldModel = new KategoriPerizinanModel.ViewModel();
                    fieldModel.id = KategoriPerizinanModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = KategoriPerizinanModel.GetViewData(PkValue);
                    KategoriPerizinanModel.ViewModel fieldModel = new KategoriPerizinanModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = KategoriPerizinanModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(KategoriPerizinanModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = KategoriPerizinanModel.GetNewId().ToString();
                    ...
                    result = KategoriPerizinanModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(KategoriPerizinanModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(KategoriPerizinanModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(KategoriPerizinanModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = KategoriPerizinanModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = KategoriPerizinanModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = KategoriPerizinanModel.LookupData(param);
            return Json(data);
        }
