...
namespace WebApp.Controllers
{
    ...
    public class PersonAreaController : Controller
    {
        ...
        private string _path_controler = "/PersonArea/";
        private string _path_view = "/Views/PersonArea/";
        private readonly string _table_name = "ta_person_area";
        private readonly string _table_title = "Area Person";

		private static IHostingEnvironment _hostingEnvironment;
        public PersonAreaController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(PersonAreaModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = PersonAreaModel._pkKey;
                    ViewData["displayItem"] = PersonAreaModel.GetDisplayItem();
                    ViewData["column_att"] = PersonAreaModel.GetGridColumn();
                    ViewData["param"] = PersonAreaModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(PersonAreaModel.ActionRequest param)
        {
            GridData data = PersonAreaModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    PersonAreaModel.ViewModel fieldModel = new PersonAreaModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = PersonAreaModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(PersonAreaModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //PersonAreaModel.ViewModel fieldModel = new PersonAreaModel.ViewModel();
                    fieldModel.id = PersonAreaModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = PersonAreaModel.GetViewData(PkValue);
                    PersonAreaModel.ViewModel fieldModel = new PersonAreaModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = PersonAreaModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(PersonAreaModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = PersonAreaModel.GetNewId().ToString();
                    ...
                    result = PersonAreaModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(PersonAreaModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(PersonAreaModel.GetData(PkValue));
                    ...
                    OrderedDictionary data = DataModel.DtToDictionary(PersonAreaModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = PersonAreaModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = PersonAreaModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = PersonAreaModel.LookupData(param);
            return Json(data);
        }
