...
namespace WebApp.Areas.Ref.Controllers
{
    ...
    public class PersonalAreaController : Controller
    {
        ...
        private string _path_controler = "/Ref/PersonalArea/";
        private string _path_view = "/Areas/Ref/Views/PersonalArea/";
        private readonly string _table_name = "ref_personal_area";
        private readonly string _table_title = "Personal Area";

		private static IHostingEnvironment _hostingEnvironment;
        public PersonalAreaController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(PersonalAreaModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = PersonalAreaModel._pkKey;
                    ViewData["displayItem"] = PersonalAreaModel.GetDisplayItem();
                    ViewData["column_att"] = PersonalAreaModel.GetGridColumn();
                    ViewData["param"] = PersonalAreaModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(PersonalAreaModel.ActionRequest param)
        {
            GridData data = PersonalAreaModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    PersonalAreaModel.ViewModel fieldModel = new PersonalAreaModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = PersonalAreaModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(PersonalAreaModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //PersonalAreaModel.ViewModel fieldModel = new PersonalAreaModel.ViewModel();
                    fieldModel.id = PersonalAreaModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = PersonalAreaModel.GetViewData(PkValue);
                    PersonalAreaModel.ViewModel fieldModel = new PersonalAreaModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = PersonalAreaModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(PersonalAreaModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = PersonalAreaModel.GetNewId().ToString();
                    ...
                    result = PersonalAreaModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(PersonalAreaModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(PersonalAreaModel.GetData(PkValue));
                    ...
                    OrderedDictionary data = DataModel.DtToDictionary(PersonalAreaModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = PersonalAreaModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = PersonalAreaModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = PersonalAreaModel.LookupData(param);
            return Json(data);
        }

        public JsonResult LookupDataPA(lookup_param param)
        {
            DataTable data = PersonalAreaModel.LookupDataPA(HttpContext, param);
            return Json(data);
        }
    }
}