...
namespace WebApp.Controllers
{
    ...
    public class FasSpekHydrantController : Controller
    {
        ...
        private string _path_controler = "/FasSpekHydrant/";
        private string _path_view = "/Views/Fasilitas/FasSpekHydrant/";
        private readonly string _table_name = "ta_fas_spek_hydrant";
        private readonly string _table_title = "Fas Spek Hydrant";

		private static IHostingEnvironment _hostingEnvironment;
        public FasSpekHydrantController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(FasSpekHydrantModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = FasSpekHydrantModel._pkKey;
                    ViewData["displayItem"] = FasSpekHydrantModel.GetDisplayItem();
                    ViewData["column_att"] = FasSpekHydrantModel.GetGridColumn();
                    ViewData["param"] = FasSpekHydrantModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(FasSpekHydrantModel.ActionRequest param)
        {
            GridData data = FasSpekHydrantModel.GetListData(param);
            string baseUrl = WebHelper.GetBaseUrl(HttpContext);
        ...
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    FasSpekHydrantModel.ViewModel fieldModel = new FasSpekHydrantModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        var PkValue = new OrderedDictionary();
                        PkValue["id"] = id;
                        data = FasSpekHydrantModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.jenis_pompa_id = dr["jenis_pompa_id"].ToString();
                                fieldModel.jenis_pompa_nama = dr["jenis_pompa_nama"].ToString();
                                fieldModel.jumlah_pompa = dr["jumlah_pompa"].ToString();
                                fieldModel.sistem_kerja_pompa_id = dr["sistem_kerja_pompa_id"].ToString();
                                fieldModel.sistem_kerja_pompa_id_text = dr["sistem_kerja_pompa_id_text"].ToString();
        ...
        [HttpGet]
        public IActionResult Add(FasSpekHydrantModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //FasSpekHydrantModel.ViewModel fieldModel = new FasSpekHydrantModel.ViewModel();
                    fieldModel.id = FasSpekHydrantModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = FasSpekHydrantModel.GetViewData(PkValue);
                    FasSpekHydrantModel.ViewModel fieldModel = new FasSpekHydrantModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = FasSpekHydrantModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(FasSpekHydrantModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = FasSpekHydrantModel.GetNewId().ToString();
                    ...
                    data["jenis_pompa_id"] = AntiXss.HtmlEncode(fieldModel.jenis_pompa_id);
                    data["jenis_pompa_nama"] = AntiXss.HtmlEncode(fieldModel.jenis_pompa_nama);
                    data["jumlah_pompa"] = fieldModel.jumlah_pompa;
                    data["sistem_kerja_pompa_id"] = fieldModel.sistem_kerja_pompa_id;
                    ...
                    result = FasSpekHydrantModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(FasSpekHydrantModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(FasSpekHydrantModel.GetData(PkValue));
                    ...
                    OrderedDictionary data = DataModel.DtToDictionary(FasSpekHydrantModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = FasSpekHydrantModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = FasSpekHydrantModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = FasSpekHydrantModel.LookupData(param);
            return Json(data);
        }
