...
namespace WebApp.Controllers
{
    ...
    public class FasSpekAlarmKebakaranController : Controller
    {
        ...
        private string _path_controler = "/FasSpekAlarmKebakaran/";
        private string _path_view = "/Views/Fasilitas/FasSpekAlarmKebakaran/";
        private readonly string _table_name = "ta_fas_spek_alarm_kebakaran";
        private readonly string _table_title = "Fas Spek Alarm Kebakaran";

		private static IHostingEnvironment _hostingEnvironment;
        public FasSpekAlarmKebakaranController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(FasSpekAlarmKebakaranModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = FasSpekAlarmKebakaranModel._pkKey;
                    ViewData["displayItem"] = FasSpekAlarmKebakaranModel.GetDisplayItem();
                    ViewData["column_att"] = FasSpekAlarmKebakaranModel.GetGridColumn();
                    ViewData["param"] = FasSpekAlarmKebakaranModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(FasSpekAlarmKebakaranModel.ActionRequest param)
        {
            GridData data = FasSpekAlarmKebakaranModel.GetListData(param);
            string baseUrl = WebHelper.GetBaseUrl(HttpContext);
        ...
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    FasSpekAlarmKebakaranModel.ViewModel fieldModel = new FasSpekAlarmKebakaranModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        var PkValue = new OrderedDictionary();
                        PkValue["id"] = id;
                        data = FasSpekAlarmKebakaranModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.jumlah_zona = dr["jumlah_zona"].ToString();
        ...
        [HttpGet]
        public IActionResult Add(FasSpekAlarmKebakaranModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //FasSpekAlarmKebakaranModel.ViewModel fieldModel = new FasSpekAlarmKebakaranModel.ViewModel();
                    fieldModel.id = FasSpekAlarmKebakaranModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = FasSpekAlarmKebakaranModel.GetViewData(PkValue);
                    FasSpekAlarmKebakaranModel.ViewModel fieldModel = new FasSpekAlarmKebakaranModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = FasSpekAlarmKebakaranModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(FasSpekAlarmKebakaranModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = FasSpekAlarmKebakaranModel.GetNewId().ToString();
                    ...
                    data["jumlah_zona"] = fieldModel.jumlah_zona;
                    ...
                    result = FasSpekAlarmKebakaranModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(FasSpekAlarmKebakaranModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(FasSpekAlarmKebakaranModel.GetData(PkValue));
                    data_old["tgl_periksa_akhir"] = data_old["tgl_periksa_akhir"].ToString() != "" ? String.Format("{0:yyyy-MM-dd}", data_old["tgl_periksa_akhir"]) : "";
                    OrderedDictionary data = DataModel.DtToDictionary(FasSpekAlarmKebakaranModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = FasSpekAlarmKebakaranModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = FasSpekAlarmKebakaranModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = FasSpekAlarmKebakaranModel.LookupData(param);
            return Json(data);
        }
