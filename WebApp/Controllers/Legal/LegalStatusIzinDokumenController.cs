...
namespace WebApp.Controllers
{
    ...
    public class LegalStatusIzinDokumenController : Controller
    {
        ...
        private string _path_controler = "/LegalStatusIzinDokumen/";
        private string _path_view = "/Views/Legal/LegalStatusIzinDokumen/";
        private readonly string _table_name = "ta_legal_status_izin_dokumen";
        private readonly string _table_title = "Legal Status Izin Dokumen";

		private static IHostingEnvironment _hostingEnvironment;
        public LegalStatusIzinDokumenController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(LegalStatusIzinDokumenModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = LegalStatusIzinDokumenModel._pkKey;
                    ViewData["displayItem"] = LegalStatusIzinDokumenModel.GetDisplayItem();
                    ViewData["column_att"] = LegalStatusIzinDokumenModel.GetGridColumn();
                    ViewData["param"] = LegalStatusIzinDokumenModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(LegalStatusIzinDokumenModel.ActionRequest param)
        {
            GridData data = LegalStatusIzinDokumenModel.GetListData(param);
            string baseUrl = WebHelper.GetBaseUrl(HttpContext);
        ...
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    LegalStatusIzinDokumenModel.ViewModel fieldModel = new LegalStatusIzinDokumenModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        var PkValue = new OrderedDictionary();
                        PkValue["id"] = id;
                        data = LegalStatusIzinDokumenModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(LegalStatusIzinDokumenModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //LegalStatusIzinDokumenModel.ViewModel fieldModel = new LegalStatusIzinDokumenModel.ViewModel();
                    fieldModel.id = LegalStatusIzinDokumenModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = LegalStatusIzinDokumenModel.GetViewData(PkValue);
                    LegalStatusIzinDokumenModel.ViewModel fieldModel = new LegalStatusIzinDokumenModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = LegalStatusIzinDokumenModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(LegalStatusIzinDokumenModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = LegalStatusIzinDokumenModel.GetNewId().ToString();
                    ...
                    result = LegalStatusIzinDokumenModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(LegalStatusIzinDokumenModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(LegalStatusIzinDokumenModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(LegalStatusIzinDokumenModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = LegalStatusIzinDokumenModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = LegalStatusIzinDokumenModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = LegalStatusIzinDokumenModel.LookupData(param);
            return Json(data);
        }
