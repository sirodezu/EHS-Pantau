...
namespace WebApp.Areas.Ref.Controllers
{
    ...
    public class JenisKegiatanController : Controller
    {
        ...
        private string _path_controler = "/Ref/JenisKegiatan/";
        private string _path_view = "/Areas/Ref/Views/JenisKegiatan/";
        private readonly string _table_name = "ref_jenis_kegiatan";
        private readonly string _table_title = "Jenis Kegiatan";

		private static IHostingEnvironment _hostingEnvironment;
        public JenisKegiatanController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(JenisKegiatanModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = JenisKegiatanModel._pkKey;
                    ViewData["displayItem"] = JenisKegiatanModel.GetDisplayItem();
                    ViewData["column_att"] = JenisKegiatanModel.GetGridColumn();
                    ViewData["param"] = JenisKegiatanModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(JenisKegiatanModel.ActionRequest param)
        {
            GridData data = JenisKegiatanModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    JenisKegiatanModel.ViewModel fieldModel = new JenisKegiatanModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = JenisKegiatanModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(JenisKegiatanModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //JenisKegiatanModel.ViewModel fieldModel = new JenisKegiatanModel.ViewModel();
                    fieldModel.id = JenisKegiatanModel.GetNewId().ToString();
                    ViewData["fieldModel"] = fieldModel;
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = JenisKegiatanModel.GetViewData(PkValue);
                    JenisKegiatanModel.ViewModel fieldModel = new JenisKegiatanModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = JenisKegiatanModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(JenisKegiatanModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = JenisKegiatanModel.GetNewId().ToString();
                    ...
                    result = JenisKegiatanModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(JenisKegiatanModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(JenisKegiatanModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(JenisKegiatanModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = JenisKegiatanModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = JenisKegiatanModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = JenisKegiatanModel.LookupData(param);
            return Json(data);
        }
