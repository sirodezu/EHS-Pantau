...
namespace WebApp.Areas.Ref.Controllers
{
    ...
    [Area("Ref")]
    public class JabatanPositionController : Controller
    {
        ...
        private string _path_controler = "/Ref/JabatanPosition/";
        private string _path_view = "/Areas/Ref/Views/JabatanPosition/";
        private readonly string _table_name = "ref_jabatan_position";
        private readonly string _table_title = "Jabatan Position";

		private static IHostingEnvironment _hostingEnvironment;
        public JabatanPositionController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(JabatanPositionModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = JabatanPositionModel._pkKey;
                    ViewData["displayItem"] = JabatanPositionModel.GetDisplayItem();
                    ViewData["column_att"] = JabatanPositionModel.GetGridColumn();
                    ViewData["param"] = JabatanPositionModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(JabatanPositionModel.ActionRequest param)
        {
            GridData data = JabatanPositionModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    JabatanPositionModel.ViewModel fieldModel = new JabatanPositionModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = JabatanPositionModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(JabatanPositionModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //JabatanPositionModel.ViewModel fieldModel = new JabatanPositionModel.ViewModel();
                    fieldModel.id = JabatanPositionModel.GetNewId().ToString();
                    ViewData["fieldModel"] = fieldModel;
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = JabatanPositionModel.GetViewData(PkValue);
                    JabatanPositionModel.ViewModel fieldModel = new JabatanPositionModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = JabatanPositionModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(JabatanPositionModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = JabatanPositionModel.GetNewId().ToString();
                    ...
                    result = JabatanPositionModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(JabatanPositionModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(JabatanPositionModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(JabatanPositionModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = JabatanPositionModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = JabatanPositionModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = JabatanPositionModel.LookupData(param);
            return Json(data);
        }
