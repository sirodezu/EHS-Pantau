...
namespace WebApp.Controllers
{
    ...
    public class AccidentSaksiController : Controller
    {
        ...
        private string _path_controler = "/AccidentSaksi/";
        private string _path_view = "/Views/Accident/AccidentSaksi/";
        private readonly string _table_name = "ta_acc_saksi";
        private readonly string _table_title = "Accident Saksi";

		private static IHostingEnvironment _hostingEnvironment;
        public AccidentSaksiController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(AccidentSaksiModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = AccidentSaksiModel._pkKey;
                    ViewData["displayItem"] = AccidentSaksiModel.GetDisplayItem();
                    ViewData["column_att"] = AccidentSaksiModel.GetGridColumn();
                    ViewData["param"] = AccidentSaksiModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(AccidentSaksiModel.ActionRequest param)
        {
        ...
        [HttpGet]
        public IActionResult Add(AccidentSaksiModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(AccidentSaksiModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = AccidentSaksiModel.GetNewId().ToString();
                    ...
                    result = AccidentSaksiModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(AccidentSaksiModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(AccidentSaksiModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(AccidentSaksiModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = AccidentSaksiModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = AccidentSaksiModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = AccidentSaksiModel.LookupData(param);
            return Json(data);
        }
