...
namespace WebApp.Controllers
{
    ...
    public class AccidentPelakuController : Controller
    {
        ...
        private string _path_controler = "/AccidentPelaku/";
        private string _path_view = "/Views/Accident/AccidentPelaku/";
        private readonly string _table_name = "ta_acc_pelaku";
        private readonly string _table_title = "Accident Pelaku";

		private static IHostingEnvironment _hostingEnvironment;
        public AccidentPelakuController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(AccidentPelakuModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = AccidentPelakuModel._pkKey;
                    ViewData["displayItem"] = AccidentPelakuModel.GetDisplayItem();
                    ViewData["column_att"] = AccidentPelakuModel.GetGridColumn();
                    ViewData["param"] = AccidentPelakuModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(AccidentPelakuModel.ActionRequest param)
        {
        ...
        [HttpGet]
        public IActionResult Add(AccidentPelakuModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(AccidentPelakuModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = AccidentPelakuModel.GetNewId().ToString();
                    ...
                    result = AccidentPelakuModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(AccidentPelakuModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(AccidentPelakuModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(AccidentPelakuModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = AccidentPelakuModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = AccidentPelakuModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = AccidentPelakuModel.LookupData(param);
            return Json(data);
        }
