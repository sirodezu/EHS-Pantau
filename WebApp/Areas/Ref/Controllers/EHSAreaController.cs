...
namespace WebApp.Areas.Ref.Controllers
{
    ...
    [Area("Ref")]
    public class EHSAreaController : Controller
    {
        ...
        private string _path_controler = "/Ref/EHSArea/";
        private string _path_view = "/Areas/Ref/Views/EHSArea/";
        private readonly string _table_name = "ref_ehs_area";
        private readonly string _table_title = "EHS Area";

		private static IHostingEnvironment _hostingEnvironment;
        public EHSAreaController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(EHSAreaModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = EHSAreaModel._pkKey;
                    ViewData["displayItem"] = EHSAreaModel.GetDisplayItem();
                    ViewData["column_att"] = EHSAreaModel.GetGridColumn();
                    ViewData["param"] = EHSAreaModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(EHSAreaModel.ActionRequest param)
        {
            GridData data = EHSAreaModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    EHSAreaModel.ViewModel fieldModel = new EHSAreaModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = EHSAreaModel.GetViewData(PkValue);
                        if (data != null)
                        {
                    ...
                    ViewData["fieldModel"] = fieldModel;
                    ViewData["HeadData"] = EHSAreaModel.GetHeadData(id);
        ...
        [HttpGet]
        public IActionResult Add(EHSAreaModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //EHSAreaModel.ViewModel fieldModel = new EHSAreaModel.ViewModel();
                    fieldModel.id = EHSAreaModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = EHSAreaModel.GetViewData(PkValue);
                    EHSAreaModel.ViewModel fieldModel = new EHSAreaModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = EHSAreaModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(EHSAreaModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = EHSAreaModel.GetNewId().ToString();
                    ...
                    result = EHSAreaModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(EHSAreaModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(EHSAreaModel.GetData(PkValue));
                    ...
                    OrderedDictionary data = DataModel.DtToDictionary(EHSAreaModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = EHSAreaModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = EHSAreaModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = EHSAreaModel.LookupData(param);
            return Json(data);
        }
        [HttpPost]
        public JsonResult LookupDataEA(lookup_param param)
        {
            DataTable data = EHSAreaModel.LookupDataEA(HttpContext, param);
            return Json(data);
        }
