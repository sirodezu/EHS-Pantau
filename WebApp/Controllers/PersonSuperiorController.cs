...
namespace WebApp.Controllers
{
    ...
    public class PersonSuperiorController : Controller
    {
        ...
        private string _path_controler = "/PersonSuperior/";
        private string _path_view = "/Views/PersonSuperior/";
        private readonly string _table_name = "ta_person_superior";
        private readonly string _table_title = "Person Superior";

		private static IHostingEnvironment _hostingEnvironment;
        public PersonSuperiorController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(PersonSuperiorModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = PersonSuperiorModel._pkKey;
                    ViewData["displayItem"] = PersonSuperiorModel.GetDisplayItem();
                    ViewData["column_att"] = PersonSuperiorModel.GetGridColumn();
                    ViewData["param"] = PersonSuperiorModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(PersonSuperiorModel.ActionRequest param)
        {
            GridData data = PersonSuperiorModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    PersonSuperiorModel.ViewModel fieldModel = new PersonSuperiorModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = PersonSuperiorModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(PersonSuperiorModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //PersonSuperiorModel.ViewModel fieldModel = new PersonSuperiorModel.ViewModel();
                    fieldModel.id = PersonSuperiorModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = PersonSuperiorModel.GetViewData(PkValue);
                    PersonSuperiorModel.ViewModel fieldModel = new PersonSuperiorModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = PersonSuperiorModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(PersonSuperiorModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = PersonSuperiorModel.GetNewId().ToString();
                    ...
                    result = PersonSuperiorModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(PersonSuperiorModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(PersonSuperiorModel.GetData(PkValue));
                    ...
                    OrderedDictionary data = DataModel.DtToDictionary(PersonSuperiorModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = PersonSuperiorModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = PersonSuperiorModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = PersonSuperiorModel.LookupData(param);
            return Json(data);
        }
