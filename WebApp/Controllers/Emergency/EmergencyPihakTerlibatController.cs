...
namespace WebApp.Controllers
{
    ...
    public class EmergencyPihakTerlibatController : Controller
    {
        ...
        private string _path_controler = "/EmergencyPihakTerlibat/";
        private string _path_view = "/Views/Emergency/EmergencyPihakTerlibat/";
        private readonly string _table_name = "ta_emg_pihak_terlibat";
        private readonly string _table_title = "Emergency Pihak Terlibat";

		private static IHostingEnvironment _hostingEnvironment;
        public EmergencyPihakTerlibatController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(EmergencyPihakTerlibatModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = EmergencyPihakTerlibatModel._pkKey;
                    ViewData["displayItem"] = EmergencyPihakTerlibatModel.GetDisplayItem();
                    ViewData["column_att"] = EmergencyPihakTerlibatModel.GetGridColumn();
                    ViewData["param"] = EmergencyPihakTerlibatModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(EmergencyPihakTerlibatModel.ActionRequest param)
        {
            GridData data = EmergencyPihakTerlibatModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    EmergencyPihakTerlibatModel.ViewModel fieldModel = new EmergencyPihakTerlibatModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = EmergencyPihakTerlibatModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(EmergencyPihakTerlibatModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //EmergencyPihakTerlibatModel.ViewModel fieldModel = new EmergencyPihakTerlibatModel.ViewModel();
                    fieldModel.id = EmergencyPihakTerlibatModel.GetNewId().ToString();
                    ViewData["fieldModel"] = fieldModel;
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = EmergencyPihakTerlibatModel.GetViewData(PkValue);
                    EmergencyPihakTerlibatModel.ViewModel fieldModel = new EmergencyPihakTerlibatModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = EmergencyPihakTerlibatModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(EmergencyPihakTerlibatModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = EmergencyPihakTerlibatModel.GetNewId().ToString();
                    ...
                    result = EmergencyPihakTerlibatModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(EmergencyPihakTerlibatModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(EmergencyPihakTerlibatModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(EmergencyPihakTerlibatModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = EmergencyPihakTerlibatModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = EmergencyPihakTerlibatModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = EmergencyPihakTerlibatModel.LookupData(param);
            return Json(data);
        }
