...
namespace WebApp.Areas.Ref.Controllers
{
    ...
    public class CompanyCategoryController : Controller
    {
        ...
        private string _path_controler = "/Ref/CompanyCategory/";
        private string _path_view = "/Areas/Ref/Views/CompanyCategory/";
        private readonly string _table_name = "ref_compcat";
        private readonly string _table_title = "Company Category";

		private static IHostingEnvironment _hostingEnvironment;
        public CompanyCategoryController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(CompanyCategoryModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = CompanyCategoryModel._pkKey;
                    ViewData["displayItem"] = CompanyCategoryModel.GetDisplayItem();
                    ViewData["column_att"] = CompanyCategoryModel.GetGridColumn();
                    ViewData["param"] = CompanyCategoryModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(CompanyCategoryModel.ActionRequest param)
        {
            GridData data = CompanyCategoryModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    CompanyCategoryModel.ViewModel fieldModel = new CompanyCategoryModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = CompanyCategoryModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.comptype_id = dr["comptype_id"].ToString();
                                fieldModel.comptype_id_text = dr["comptype_id_text"].ToString();
        ...
        [HttpGet]
        public IActionResult Add(CompanyCategoryModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //CompanyCategoryModel.ViewModel fieldModel = new CompanyCategoryModel.ViewModel();
                    fieldModel.id = CompanyCategoryModel.GetNewId().ToString();
                    ViewData["fieldModel"] = fieldModel;
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = CompanyCategoryModel.GetViewData(PkValue);
                    CompanyCategoryModel.ViewModel fieldModel = new CompanyCategoryModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = CompanyCategoryModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(CompanyCategoryModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = CompanyCategoryModel.GetNewId().ToString();
                    data["comptype_id"] = fieldModel.comptype_id;
                    ...
                    result = CompanyCategoryModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(CompanyCategoryModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(CompanyCategoryModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(CompanyCategoryModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = CompanyCategoryModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = CompanyCategoryModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = CompanyCategoryModel.LookupData(param);
            return Json(data);
        }
