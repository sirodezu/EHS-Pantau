...
namespace WebApp.Controllers
{
    ...
    public class EmergencyDokumenController : Controller
    {
        ...
        private string _path_controler = "/EmergencyDokumen/";
        private string _path_view = "/Views/Emergency/EmergencyDokumen/";
        private readonly string _table_name = "ta_emg_dokumen";
        private readonly string _table_title = "Emergency Dokumen";

		private static IHostingEnvironment _hostingEnvironment;
        public EmergencyDokumenController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(EmergencyDokumenModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = EmergencyDokumenModel._pkKey;
                    ViewData["displayItem"] = EmergencyDokumenModel.GetDisplayItem();
                    ViewData["column_att"] = EmergencyDokumenModel.GetGridColumn();
                    ViewData["param"] = EmergencyDokumenModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(EmergencyDokumenModel.ActionRequest param)
        {
            GridData data = EmergencyDokumenModel.GetListData(param);
            string baseUrl = WebHelper.GetBaseUrl(HttpContext);
        ...
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    EmergencyDokumenModel.ViewModel fieldModel = new EmergencyDokumenModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        var PkValue = new OrderedDictionary();
                        PkValue["id"] = id;
                        data = EmergencyDokumenModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(EmergencyDokumenModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //EmergencyDokumenModel.ViewModel fieldModel = new EmergencyDokumenModel.ViewModel();
                    fieldModel.id = EmergencyDokumenModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = EmergencyDokumenModel.GetViewData(PkValue);
                    EmergencyDokumenModel.ViewModel fieldModel = new EmergencyDokumenModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = EmergencyDokumenModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(EmergencyDokumenModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = EmergencyDokumenModel.GetNewId().ToString();
                    ...
                    result = EmergencyDokumenModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(EmergencyDokumenModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(EmergencyDokumenModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(EmergencyDokumenModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = EmergencyDokumenModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = EmergencyDokumenModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = EmergencyDokumenModel.LookupData(param);
            return Json(data);
        }
