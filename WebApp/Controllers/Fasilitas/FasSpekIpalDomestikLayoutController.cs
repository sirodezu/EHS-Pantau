...
namespace WebApp.Controllers
{
    ...
    public class FasSpekIpalDomestikLayoutController : Controller
    {
        ...
        private string _path_controler = "/FasSpekIpalDomestikLayout/";
        private string _path_view = "/Views/Fasilitas/FasSpekIpalDomestikLayout/";
        private readonly string _table_name = "ta_fas_spek_ipal_domestik_layout";
        private readonly string _table_title = "Fas Spek Ipal Domestik Layout";

		private static IHostingEnvironment _hostingEnvironment;
        public FasSpekIpalDomestikLayoutController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(FasSpekIpalDomestikLayoutModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = FasSpekIpalDomestikLayoutModel._pkKey;
                    ViewData["displayItem"] = FasSpekIpalDomestikLayoutModel.GetDisplayItem();
                    ViewData["column_att"] = FasSpekIpalDomestikLayoutModel.GetGridColumn();
                    ViewData["param"] = FasSpekIpalDomestikLayoutModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(FasSpekIpalDomestikLayoutModel.ActionRequest param)
        {
            GridData data = FasSpekIpalDomestikLayoutModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    FasSpekIpalDomestikLayoutModel.ViewModel fieldModel = new FasSpekIpalDomestikLayoutModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = FasSpekIpalDomestikLayoutModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(FasSpekIpalDomestikLayoutModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //FasSpekIpalDomestikLayoutModel.ViewModel fieldModel = new FasSpekIpalDomestikLayoutModel.ViewModel();
                    fieldModel.id = FasSpekIpalDomestikLayoutModel.GetNewId().ToString();
                    ViewData["fieldModel"] = fieldModel;
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = FasSpekIpalDomestikLayoutModel.GetViewData(PkValue);
                    FasSpekIpalDomestikLayoutModel.ViewModel fieldModel = new FasSpekIpalDomestikLayoutModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = FasSpekIpalDomestikLayoutModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(FasSpekIpalDomestikLayoutModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = FasSpekIpalDomestikLayoutModel.GetNewId().ToString();
                    ...
                    result = FasSpekIpalDomestikLayoutModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(FasSpekIpalDomestikLayoutModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(FasSpekIpalDomestikLayoutModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(FasSpekIpalDomestikLayoutModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = FasSpekIpalDomestikLayoutModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = FasSpekIpalDomestikLayoutModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = FasSpekIpalDomestikLayoutModel.LookupData(param);
            return Json(data);
        }
