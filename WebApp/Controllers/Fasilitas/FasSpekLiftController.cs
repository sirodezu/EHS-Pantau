...
namespace WebApp.Controllers
{
    ...
    public class FasSpekLiftController : Controller
    {
        ...
        private string _path_controler = "/FasSpekLift/";
        private string _path_view = "/Views/Fasilitas/FasSpekLift/";
        private readonly string _table_name = "ta_fas_spek_lift";
        private readonly string _table_title = "Fas Spek Lift";

		private static IHostingEnvironment _hostingEnvironment;
        public FasSpekLiftController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(FasSpekLiftModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = FasSpekLiftModel._pkKey;
                    ViewData["displayItem"] = FasSpekLiftModel.GetDisplayItem();
                    ViewData["column_att"] = FasSpekLiftModel.GetGridColumn();
                    ViewData["param"] = FasSpekLiftModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(FasSpekLiftModel.ActionRequest param)
        {
            GridData data = FasSpekLiftModel.GetListData(param);
            string baseUrl = WebHelper.GetBaseUrl(HttpContext);
        ...
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    FasSpekLiftModel.ViewModel fieldModel = new FasSpekLiftModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        var PkValue = new OrderedDictionary();
                        PkValue["id"] = id;
                        data = FasSpekLiftModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.model = dr["model"].ToString();
                                fieldModel.kapasitas_tertera_orang = dr["kapasitas_tertera_orang"].ToString();
                                fieldModel.kapasitas_tertera_kg = String.Format("{0:N2}", dr["kapasitas_tertera_kg"]);
                                fieldModel.melayani_ointu = dr["melayani_ointu"].ToString();
                                fieldModel.melayani_lantai = dr["melayani_lantai"].ToString();
                                fieldModel.kecepatan = String.Format("{0:N2}", dr["kecepatan"]);
        ...
        [HttpGet]
        public IActionResult Add(FasSpekLiftModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //FasSpekLiftModel.ViewModel fieldModel = new FasSpekLiftModel.ViewModel();
                    fieldModel.id = FasSpekLiftModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = FasSpekLiftModel.GetViewData(PkValue);
                    FasSpekLiftModel.ViewModel fieldModel = new FasSpekLiftModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = FasSpekLiftModel.GetNewId().ToString();
                        ...
                        fieldModel.model = dr["model"].ToString();
                        fieldModel.kapasitas_tertera_orang = dr["kapasitas_tertera_orang"].ToString();
                        fieldModel.kapasitas_tertera_kg = dr["kapasitas_tertera_kg"].ToString();
                        fieldModel.melayani_ointu = dr["melayani_ointu"].ToString();
                        fieldModel.melayani_lantai = dr["melayani_lantai"].ToString();
                        fieldModel.kecepatan = dr["kecepatan"].ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(FasSpekLiftModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = FasSpekLiftModel.GetNewId().ToString();
                    ...
                    data["model"] = AntiXss.HtmlEncode(fieldModel.model);
                    data["kapasitas_tertera_orang"] = fieldModel.kapasitas_tertera_orang;
                    data["kapasitas_tertera_kg"] = fieldModel.kapasitas_tertera_kg != null ? fieldModel.kapasitas_tertera_kg.Replace(",",".") : "";
                    data["melayani_ointu"] = AntiXss.HtmlEncode(fieldModel.melayani_ointu);
                    data["melayani_lantai"] = fieldModel.melayani_lantai;
                    data["kecepatan"] = fieldModel.kecepatan != null ? fieldModel.kecepatan.Replace(",",".") : "";
                    ...
                    result = FasSpekLiftModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(FasSpekLiftModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(FasSpekLiftModel.GetData(PkValue));
                    data_old["kapasitas_tertera_kg"] = data_old["kapasitas_tertera_kg"].ToString() != ""  ? data_old["kapasitas_tertera_kg"].ToString().Replace(",", ".") : "";
                    data_old["kecepatan"] = data_old["kecepatan"].ToString() != ""  ? data_old["kecepatan"].ToString().Replace(",", ".") : "";
                    data_old["tgl_periksa_akhir"] = data_old["tgl_periksa_akhir"].ToString() != "" ? String.Format("{0:yyyy-MM-dd}", data_old["tgl_periksa_akhir"]) : "";
                    OrderedDictionary data = DataModel.DtToDictionary(FasSpekLiftModel.GetData(PkValue));
                    ...
                    data["model"] = AntiXss.HtmlEncode(fieldModel.model);
                    data["kapasitas_tertera_orang"] = fieldModel.kapasitas_tertera_orang;
                    data["kapasitas_tertera_kg"] = fieldModel.kapasitas_tertera_kg != null ? fieldModel.kapasitas_tertera_kg.ToString().Replace(",", ".") : "";
                    data["melayani_ointu"] = AntiXss.HtmlEncode(fieldModel.melayani_ointu);
                    data["melayani_lantai"] = fieldModel.melayani_lantai;
                    data["kecepatan"] = fieldModel.kecepatan != null ? fieldModel.kecepatan.ToString().Replace(",", ".") : "";
                    data["photo"] = AntiXss.HtmlEncode(fieldModel.photo);
                    data["tgl_periksa_akhir"] = fieldModel.tgl_periksa_akhir;
                    data["hasil_periksa_akhir"] = fieldModel.hasil_periksa_akhir;
                    SqlHelper.ConvertEmptyValuesToDBNull(data);
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = FasSpekLiftModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = FasSpekLiftModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = FasSpekLiftModel.LookupData(param);
            return Json(data);
        }
