...
namespace WebApp.Areas.Ref.Controllers
{
    ...
    public class OrgUnitController : Controller
    {
        ...
        private string _path_controler = "/Ref/OrgUnit/";
        private string _path_view = "/Areas/Ref/Views/OrgUnit/";
        private readonly string _table_name = "ref_org_unit";
        private readonly string _table_title = "Organization Unit";

		private static IHostingEnvironment _hostingEnvironment;
        public OrgUnitController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(OrgUnitModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = OrgUnitModel._pkKey;
                    ViewData["displayItem"] = OrgUnitModel.GetDisplayItem();
                    ViewData["column_att"] = OrgUnitModel.GetGridColumn();
                    ViewData["param"] = OrgUnitModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(OrgUnitModel.ActionRequest param)
        {
            GridData data = OrgUnitModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    OrgUnitModel.ViewModel fieldModel = new OrgUnitModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = OrgUnitModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(OrgUnitModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //OrgUnitModel.ViewModel fieldModel = new OrgUnitModel.ViewModel();
                    fieldModel.id = OrgUnitModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = OrgUnitModel.GetViewData(PkValue);
                    OrgUnitModel.ViewModel fieldModel = new OrgUnitModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = OrgUnitModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(OrgUnitModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = OrgUnitModel.GetNewId().ToString();
                    ...
                    result = OrgUnitModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(OrgUnitModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(OrgUnitModel.GetData(PkValue));
                    ...
                    OrderedDictionary data = DataModel.DtToDictionary(OrgUnitModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = OrgUnitModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = OrgUnitModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = OrgUnitModel.LookupData(param);
            return Json(data);
        }
