...
namespace WebApp.Areas.Ref.Controllers
{
    ...
    public class RoomController : Controller
    {
        ...
        private string _path_controler = "/Ref/Room/";
        private string _path_view = "/Areas/Ref/Views/Room/";
        private readonly string _table_name = "ref_room";
        private readonly string _table_title = "Ruangan";
		...
		[HttpGet]
        public IActionResult Index(RoomModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = RoomModel._pkKey;
                    ViewData["displayItem"] = RoomModel.GetDisplayItem();
                    ViewData["column_att"] = RoomModel.GetGridColumn();
                    ViewData["param"] = RoomModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(RoomModel.ActionRequest param)
        {
            GridData data = RoomModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    RoomModel.ViewModel fieldModel = new RoomModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = RoomModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.loc_id = dr["loc_id"].ToString();
                                fieldModel.loc_id_text = dr["loc_id_text"].ToString();
        ...
        [HttpGet]
        public IActionResult Add(RoomModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //RoomModel.ViewModel fieldModel = new RoomModel.ViewModel();
                    fieldModel.id = RoomModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = RoomModel.GetViewData(PkValue);
                    RoomModel.ViewModel fieldModel = new RoomModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = RoomModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(RoomModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = RoomModel.GetNewId().ToString();
                    data["loc_id"] = fieldModel.loc_id;
                    ...
                    result = RoomModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(RoomModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(RoomModel.GetData(PkValue));
                    ...
                    OrderedDictionary data = DataModel.DtToDictionary(RoomModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = RoomModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = RoomModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = RoomModel.LookupData(param);
            return Json(data);
        }
