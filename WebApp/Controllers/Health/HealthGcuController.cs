...
namespace WebApp.Controllers
{
    ...
    public class HealthGcuController : Controller
    {
        ...
        private string _path_controler = "/HealthGcu/";
        private string _path_view = "/Views/Health/HealthGcu/";
        private readonly string _table_name = "ta_health_gcu";
        private readonly string _table_title = "Health Gcu";

		private static IHostingEnvironment _hostingEnvironment;
        public HealthGcuController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(HealthGcuModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = HealthGcuModel._pkKey;
                    ViewData["displayItem"] = HealthGcuModel.GetDisplayItem();
                    ViewData["column_att"] = HealthGcuModel.GetGridColumn();
                    ViewData["param"] = HealthGcuModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(HealthGcuModel.ActionRequest param)
        {
            GridData data = HealthGcuModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    HealthGcuModel.ViewModel fieldModel = new HealthGcuModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = HealthGcuModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.gcu_result_id = dr["gcu_result_id"].ToString();
                                fieldModel.gcu_result_id_text = dr["gcu_result_id_text"].ToString();
                                fieldModel.gcu_result_normal_id = dr["gcu_result_normal_id"].ToString();
                                fieldModel.gcu_result_normal_id_text = dr["gcu_result_normal_id_text"].ToString();
        ...
        [HttpGet]
        public IActionResult Add(HealthGcuModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //HealthGcuModel.ViewModel fieldModel = new HealthGcuModel.ViewModel();
                    fieldModel.id = HealthGcuModel.GetNewId().ToString();
                    ViewData["fieldModel"] = fieldModel;
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = HealthGcuModel.GetViewData(PkValue);
                    HealthGcuModel.ViewModel fieldModel = new HealthGcuModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = HealthGcuModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(HealthGcuModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = HealthGcuModel.GetNewId().ToString();
                    data["health_id"] = fieldModel.health_id;
                    data["gcu_result_id"] = fieldModel.gcu_result_id;
                    data["gcu_result_normal_id"] = fieldModel.gcu_result_normal_id;
                    ...
                    result = HealthGcuModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(HealthGcuModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(HealthGcuModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(HealthGcuModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = HealthGcuModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = HealthGcuModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = HealthGcuModel.LookupData(param);
            return Json(data);
        }
