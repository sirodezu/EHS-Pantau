...
namespace WebApp.Areas.Ref.Controllers
{
    ...
    public class LocationController : Controller
    {
        ...
        private string _path_controler = "/Ref/Location/";
        private string _path_view = "/Areas/Ref/Views/Location/";
        private readonly string _table_name = "ref_location";
        private readonly string _table_title = "Location";
		...
		[HttpGet]
        public IActionResult Index(LocationModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = LocationModel._pkKey;
                    ViewData["displayItem"] = LocationModel.GetDisplayItem();
                    ViewData["column_att"] = LocationModel.GetGridColumn();
                    ViewData["param"] = LocationModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(LocationModel.ActionRequest param)
        {
            GridData data = LocationModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    LocationModel.ViewModel fieldModel = new LocationModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = LocationModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(LocationModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //LocationModel.ViewModel fieldModel = new LocationModel.ViewModel();
                    fieldModel.id = LocationModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = LocationModel.GetViewData(PkValue);
                    LocationModel.ViewModel fieldModel = new LocationModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = LocationModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(LocationModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = LocationModel.GetNewId().ToString();
                    ...
                    result = LocationModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(LocationModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(LocationModel.GetData(PkValue));
                    ...
                    OrderedDictionary data = DataModel.DtToDictionary(LocationModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = LocationModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = LocationModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = LocationModel.LookupData(param);
            return Json(data);
        }
