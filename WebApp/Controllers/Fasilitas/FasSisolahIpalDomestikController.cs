...
namespace WebApp.Controllers
{
    ...
    public class FasSisolahIpalDomestikController : Controller
    {
        ...
        private string _path_controler = "/FasSisolahIpalDomestik/";
        private string _path_view = "/Views/Fasilitas/FasSisolahIpalDomestik/";
        private readonly string _table_name = "ta_fas_sisolah_ipal_domestik";
        private readonly string _table_title = "Fas Sisolah Ipal Domestik";

		private static IHostingEnvironment _hostingEnvironment;
        public FasSisolahIpalDomestikController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(FasSisolahIpalDomestikModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = FasSisolahIpalDomestikModel._pkKey;
                    ViewData["displayItem"] = FasSisolahIpalDomestikModel.GetDisplayItem();
                    ViewData["column_att"] = FasSisolahIpalDomestikModel.GetGridColumn();
                    ViewData["param"] = FasSisolahIpalDomestikModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(FasSisolahIpalDomestikModel.ActionRequest param)
        {
            GridData data = FasSisolahIpalDomestikModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    FasSisolahIpalDomestikModel.ViewModel fieldModel = new FasSisolahIpalDomestikModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = FasSisolahIpalDomestikModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.spek_ipal_domestik_id = dr["spek_ipal_domestik_id"].ToString();
                                fieldModel.sistem_pengolahan_ipal_domestik_id = dr["sistem_pengolahan_ipal_domestik_id"].ToString();
        ...
        [HttpGet]
        public IActionResult Add(FasSisolahIpalDomestikModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //FasSisolahIpalDomestikModel.ViewModel fieldModel = new FasSisolahIpalDomestikModel.ViewModel();
                    fieldModel.id = FasSisolahIpalDomestikModel.GetNewId().ToString();
                    ViewData["fieldModel"] = fieldModel;
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = FasSisolahIpalDomestikModel.GetViewData(PkValue);
                    FasSisolahIpalDomestikModel.ViewModel fieldModel = new FasSisolahIpalDomestikModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = FasSisolahIpalDomestikModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(FasSisolahIpalDomestikModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = FasSisolahIpalDomestikModel.GetNewId().ToString();
                    data["spek_ipal_domestik_id"] = fieldModel.spek_ipal_domestik_id;
                    data["sistem_pengolahan_ipal_domestik_id"] = fieldModel.sistem_pengolahan_ipal_domestik_id;
                    ...
                    result = FasSisolahIpalDomestikModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(FasSisolahIpalDomestikModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(FasSisolahIpalDomestikModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(FasSisolahIpalDomestikModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = FasSisolahIpalDomestikModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = FasSisolahIpalDomestikModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = FasSisolahIpalDomestikModel.LookupData(param);
            return Json(data);
        }
