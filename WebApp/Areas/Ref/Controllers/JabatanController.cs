...
namespace WebApp.Areas.Ref.Controllers
{
    ...
    public class JabatanController : Controller
    {
        ...
        private string _path_controler = "/Ref/Jabatan/";
        private string _path_view = "/Areas/Ref/Views/Jabatan/";
        private readonly string _table_name = "ref_jabatan";
        private readonly string _table_title = "Jabatan";

		private static IHostingEnvironment _hostingEnvironment;
        public JabatanController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(JabatanModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = JabatanModel._pkKey;
                    ViewData["displayItem"] = JabatanModel.GetDisplayItem();
                    ViewData["column_att"] = JabatanModel.GetGridColumn();
                    ViewData["param"] = JabatanModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(JabatanModel.ActionRequest param)
        {
            GridData data = JabatanModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    JabatanModel.ViewModel fieldModel = new JabatanModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = JabatanModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(JabatanModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //JabatanModel.ViewModel fieldModel = new JabatanModel.ViewModel();
                    fieldModel.id = JabatanModel.GetNewId().ToString();
                    ViewData["fieldModel"] = fieldModel;
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = JabatanModel.GetViewData(PkValue);
                    JabatanModel.ViewModel fieldModel = new JabatanModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = JabatanModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(JabatanModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = JabatanModel.GetNewId().ToString();
                    ...
                    result = JabatanModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(JabatanModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(JabatanModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(JabatanModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = JabatanModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = JabatanModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = JabatanModel.LookupData(param);
            return Json(data);
        }
