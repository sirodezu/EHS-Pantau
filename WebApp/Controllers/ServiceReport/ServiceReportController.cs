...
namespace WebApp.Controllers
{
    ...
    {
        private string _rule_view = "DashboardServiceReport";
        private string _rule_add = "DashboardServiceReport";
        private string _rule_edit = "DashboardServiceReport";
        private string _rule_delete = "DashboardServiceReport";
        private string _rule_edit_all = "DashboardServiceReport";
        private string _rule_delete_all = "DashboardServiceReport";
        private string _path_controler = "/ServiceReport/";
        private string _path_view = "/Views/ServiceReport/ServiceReport/";
        private readonly string _table_name = "ta_service_report";
        private readonly string _table_title = "Service Report";
		...
		[HttpGet]
        public IActionResult Index(ServiceReportModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = ServiceReportModel._pkKey;
                    ViewData["displayItem"] = ServiceReportModel.GetDisplayItem();
                    ViewData["column_att"] = ServiceReportModel.GetGridColumn();
                    ViewData["param"] = ServiceReportModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(ServiceReportModel.ActionRequest param)
        {
            GridData data = ServiceReportModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    ServiceReportModel.ViewModel fieldModel = new ServiceReportModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = ServiceReportModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(ServiceReportModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //ServiceReportModel.ViewModel fieldModel = new ServiceReportModel.ViewModel();
                    fieldModel.id = ServiceReportModel.GetNewId().ToString();
                    ViewData["fieldModel"] = fieldModel;
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = ServiceReportModel.GetViewData(PkValue);
                    ServiceReportModel.ViewModel fieldModel = new ServiceReportModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = ServiceReportModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(ServiceReportModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = ServiceReportModel.GetNewId().ToString();
                    ...
                    result = ServiceReportModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(ServiceReportModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(ServiceReportModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(ServiceReportModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = ServiceReportModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = ServiceReportModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = ServiceReportModel.LookupData(param);
            return Json(data);
        }


        [HttpPost]
        public JsonResult GetValidData(string cur_bulan, string cur_tahun)
        {
            DataSet data = ServiceReportModel.GetValidData(cur_bulan, cur_tahun);
            return Json(data);
        }
    }
}