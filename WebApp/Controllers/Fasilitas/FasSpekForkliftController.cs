...
namespace WebApp.Controllers
{
    ...
    public class FasSpekForkliftController : Controller
    {
        ...
        private string _rule_edit_all = "FasilitasEditAll"; //FasilitasEditAll
        private string _rule_delete_all = "FasilitasDeleteAll";
        private string _path_controler = "/FasSpekForklift/";
        private string _path_view = "/Views/Fasilitas/FasSpekForklift/";
        private readonly string _table_name = "ta_fas_spek_forklift";
        private readonly string _table_title = "Forklift";

		private static IHostingEnvironment _hostingEnvironment;
        public FasSpekForkliftController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(FasSpekForkliftModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = FasSpekForkliftModel._pkKey;
                    ViewData["displayItem"] = FasSpekForkliftModel.GetDisplayItem();
                    ViewData["column_att"] = FasSpekForkliftModel.GetGridColumn();
                    ViewData["param"] = FasSpekForkliftModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(FasSpekForkliftModel.ActionRequest param)
        {
            GridData data = FasSpekForkliftModel.GetListData(param);
            string baseUrl = WebHelper.GetBaseUrl(HttpContext);
        ...
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    FasSpekForkliftModel.ViewModel fieldModel = new FasSpekForkliftModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        var PkValue = new OrderedDictionary();
                        PkValue["id"] = id;
                        data = FasSpekForkliftModel.GetViewData(PkValue);
                        if (data != null)
                        {
        ...
        [HttpGet]
        public IActionResult Add(FasSpekForkliftModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //FasSpekForkliftModel.ViewModel fieldModel = new FasSpekForkliftModel.ViewModel();
                    fieldModel.id = FasSpekForkliftModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = FasSpekForkliftModel.GetViewData(PkValue);
                    FasSpekForkliftModel.ViewModel fieldModel = new FasSpekForkliftModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = FasSpekForkliftModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(FasSpekForkliftModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = FasSpekForkliftModel.GetNewId().ToString();
                    ...
                    result = FasSpekForkliftModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(FasSpekForkliftModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(FasSpekForkliftModel.GetData(PkValue));
                    ...
                    OrderedDictionary data = DataModel.DtToDictionary(FasSpekForkliftModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = FasSpekForkliftModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = FasSpekForkliftModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = FasSpekForkliftModel.LookupData(param);
            return Json(data);
        }
