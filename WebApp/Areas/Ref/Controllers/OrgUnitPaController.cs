...
namespace WebApp.Areas.Ref.Controllers
{
    ...
    [Area("Ref")]
    public class OrgUnitPaController : Controller
    {
        ...
        private string _path_controler = "/Ref/OrgUnitPa/";
        private string _path_view = "/Areas/Ref/Views/OrgUnitPa/";
        private readonly string _table_name = "ref_org_unit_pa";
        private readonly string _table_title = "Organization Unit vs Personal Area";

		private static IHostingEnvironment _hostingEnvironment;
        public OrgUnitPaController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(OrgUnitPaModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = OrgUnitPaModel._pkKey;
                    ViewData["displayItem"] = OrgUnitPaModel.GetDisplayItem();
                    ViewData["column_att"] = OrgUnitPaModel.GetGridColumn();
                    ViewData["param"] = OrgUnitPaModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(OrgUnitPaModel.ActionRequest param)
        {
            GridData data = OrgUnitPaModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    OrgUnitPaModel.ViewModel fieldModel = new OrgUnitPaModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = OrgUnitPaModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.org_id = dr["org_id"].ToString();
                                fieldModel.org_id_text = dr["org_id_text"].ToString();
        ...
        [HttpGet]
        public IActionResult Add(OrgUnitPaModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //OrgUnitPaModel.ViewModel fieldModel = new OrgUnitPaModel.ViewModel();
                    fieldModel.id = OrgUnitPaModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = OrgUnitPaModel.GetViewData(PkValue);
                    OrgUnitPaModel.ViewModel fieldModel = new OrgUnitPaModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = OrgUnitPaModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(OrgUnitPaModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = OrgUnitPaModel.GetNewId().ToString();
                    data["org_id"] = fieldModel.org_id;
                    ...
                    result = OrgUnitPaModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(OrgUnitPaModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(OrgUnitPaModel.GetData(PkValue));
                    ...
                    OrderedDictionary data = DataModel.DtToDictionary(OrgUnitPaModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = OrgUnitPaModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = OrgUnitPaModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = OrgUnitPaModel.LookupData(param);
            return Json(data);
        }
