...
namespace WebApp.Controllers
{
    ...
    public class SdaTisuController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> ImportSdaTisu()
        {
            ...
            List<ImportSdaTisuModel> data = await GetListData(fullPath);

            return Created(string.Empty, null);
        }

        private async Task<List<ImportSdaTisuModel>> GetListData(string fileName)
        {
            List<ImportSdaTisuModel> data = new List<ImportSdaTisuModel>();
            try
            {
                using (XLWorkbook workBook = new XLWorkbook(fileName))
                {
                    ...
                    for (int i = 1; i < range.RowCount() + 1; i++)
                    {
                        if(i == 1)
                        {
                            ...
                            if (row.Cell(i, 7).Value.ToString().Trim().ToLower() != "konsumsi tisu (kg)")
                            {
                        ...
                        {
                            ImportSdaTisuModel sda = new ImportSdaTisuModel();
                            ...
                            sda.konsumsi_tisu = double.Parse(row.Cell(i, 7).Value.ToString());
                            sda.usaha_pengurangan_tisu_id = row.Cell(i, 8).Value.ToString();
                            sda.usaha_pengurangan_tisu_desc = row.Cell(i, 9).Value.ToString();
                            sda.usaha_pengurangan_tisu_desc_file_path = row.Cell(i, 10).Value.ToString();
                            sda.usaha_pengurangan_tisu_jumlah = double.Parse(row.Cell(i, 11).Value.ToString());
                            ImportRepository sdaTisu = new ImportRepository();
                            sdaTisu.ImportSdaTisu(sda);
                        }
                    }
                    return data;
                }
