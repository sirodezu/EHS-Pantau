...
namespace WebApp.Controllers
{
    ...
    public class CampaignController : Controller
    {
        ...
        private string _path_controler = "/Campaign/";
        private string _path_view = "/Views/Campaign/Campaign/";
        private readonly string _table_name = "ta_cmp";
        private readonly string _table_title = "Campaign";
		...
		[HttpGet]
        public IActionResult Index(CampaignModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = CampaignModel._pkKey;
                    ViewData["displayItem"] = CampaignModel.GetDisplayItem();
                    ViewData["column_att"] = CampaignModel.GetGridColumn();
                    ViewData["param"] = CampaignModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(CampaignModel.ActionRequest param)
        {
            GridData data = CampaignModel.GetListData(HttpContext, param);
            string baseUrl = WebHelper.GetBaseUrl(HttpContext);
            for (int i=0; i<data.data.Rows.Count; i++) {
                var PkValue = new OrderedDictionary();
                PkValue["id"] = data.data.Rows[i]["id"];
                data.data.Rows[i]["lampiran_kegiatan"] = FileHelper.GetlinkDownload(baseUrl, _table_name, PkValue, "lampiran_kegiatan", data.data.Rows[i]["lampiran_kegiatan"].ToString());
            }
        ...
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    CampaignModel.ViewModel fieldModel = new CampaignModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        var PkValue = new OrderedDictionary();
                        PkValue["id"] = id;
                        data = CampaignModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.materi_acara = dr["materi_acara"].ToString();
                                fieldModel.keterlibatan_manajemen_id = dr["keterlibatan_manajemen_id"].ToString();
                                fieldModel.keterlibatan_manajemen_id_text = dr["keterlibatan_manajemen_id_text"].ToString();
                                fieldModel.jumlah_peserta = dr["jumlah_peserta"].ToString();
                                fieldModel.lampiran_kegiatan = FileHelper.GetlinkDownload(baseUrl, _table_name, PkValue, "lampiran_kegiatan", dr["lampiran_kegiatan"].ToString());
        ...
        [HttpGet]
        public IActionResult Add(CampaignModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //CampaignModel.ViewModel fieldModel = new CampaignModel.ViewModel();
                    fieldModel.id = CampaignModel.GetNewId().ToString();
                    fieldModel.lampiran_kegiatan_init = "[]";
                    ViewData["fieldModel"] = fieldModel;
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = CampaignModel.GetViewData(PkValue);
                    CampaignModel.ViewModel fieldModel = new CampaignModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = CampaignModel.GetNewId().ToString();
                        ...
                        fieldModel.keterlibatan_manajemen_id = dr["keterlibatan_manajemen_id"].ToString();
                        fieldModel.jumlah_peserta = dr["jumlah_peserta"].ToString();
                        fieldModel.lampiran_kegiatan = dr["lampiran_kegiatan"].ToString();
                        fieldModel.insert_by = dr["insert_by"].ToString();
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    foreach (DataRow dr in data.Rows)
                    {
                        ...
                        fieldModel.keterlibatan_manajemen_id_text = dr["keterlibatan_manajemen_id_text"].ToString();
                        fieldModel.jumlah_peserta = dr["jumlah_peserta"].ToString();
                        string[] init_lampiran_kegiatan = FileHelper.GetinitialFiles(_hostingEnvironment, HttpContext, _table_name, "lampiran_kegiatan", PkValue, dr["lampiran_kegiatan"].ToString());
                        fieldModel.lampiran_kegiatan = init_lampiran_kegiatan[0];
                        fieldModel.lampiran_kegiatan_init = init_lampiran_kegiatan[1];
                        fieldModel.insert_by = dr["insert_by"].ToString();
                        fieldModel.insert_at = dr["insert_at"].ToString();
                        fieldModel.update_by = dr["update_by"].ToString();
                        fieldModel.update_at = dr["update_at"].ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(CampaignModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = CampaignModel.GetNewId().ToString();
                    ...
                    data["materi_acara"] = AntiXss.HtmlEncode(fieldModel.materi_acara);
                    data["keterlibatan_manajemen_id"] = fieldModel.keterlibatan_manajemen_id;
                    data["jumlah_peserta"] = fieldModel.jumlah_peserta;
                    data["lampiran_kegiatan"] = AntiXss.HtmlEncode(fieldModel.lampiran_kegiatan);
                    ...
                    result = CampaignModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(CampaignModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(CampaignModel.GetData(PkValue));
                    data_old["tgl_kegiatan"] = data_old["tgl_kegiatan"].ToString() != "" ? String.Format("{0:yyyy-MM-dd}", data_old["tgl_kegiatan"]) : "";
                    OrderedDictionary data = DataModel.DtToDictionary(CampaignModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = CampaignModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = CampaignModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = CampaignModel.LookupData(param);
            return Json(data);
        }
        public async Task<IActionResult> save_lampiran_kegiatan(IEnumerable<IFormFile> lampiran_kegiatan_file)
        {
            ...
            string pathData = Path.Combine(upload_temp, sessid); pathData = Path.Combine(pathData, _table_name); pathData = Path.Combine(pathData, "lampiran_kegiatan");
            if (lampiran_kegiatan_file != null)
            {
                foreach (var file in lampiran_kegiatan_file)
                {
        ...
        public ActionResult remove_lampiran_kegiatan(string[] fileNames)
        {
            string upload_temp = Settings.GetAppSetting("path_upload_temp") != null ? Settings.GetAppSetting("path_upload_temp") : "C:\\Temp";
            string sessid = HttpContext.Session.Id;
            string pathData = Path.Combine(upload_temp, sessid); pathData = Path.Combine(pathData, _table_name); pathData = Path.Combine(pathData, "lampiran_kegiatan");
