...
namespace WebApp.Controllers
{
    ...
    public class FasSisolahIpalProduksiController : Controller
    {
        ...
        private string _path_controler = "/FasSisolahIpalProduksi/";
        private string _path_view = "/Views/Fasilitas/FasSisolahIpalProduksi/";
        private readonly string _table_name = "ta_fas_sisolah_ipal_produksi";
        private readonly string _table_title = "Fas Sisolah Ipal Produksi";

		private static IHostingEnvironment _hostingEnvironment;
        public FasSisolahIpalProduksiController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(FasSisolahIpalProduksiModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = FasSisolahIpalProduksiModel._pkKey;
                    ViewData["displayItem"] = FasSisolahIpalProduksiModel.GetDisplayItem();
                    ViewData["column_att"] = FasSisolahIpalProduksiModel.GetGridColumn();
                    ViewData["param"] = FasSisolahIpalProduksiModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(FasSisolahIpalProduksiModel.ActionRequest param)
        {
            GridData data = FasSisolahIpalProduksiModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    FasSisolahIpalProduksiModel.ViewModel fieldModel = new FasSisolahIpalProduksiModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = FasSisolahIpalProduksiModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.sistem_pengolahan_ipal_produksi_id = dr["sistem_pengolahan_ipal_produksi_id"].ToString();
        ...
        [HttpGet]
        public IActionResult Add(FasSisolahIpalProduksiModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //FasSisolahIpalProduksiModel.ViewModel fieldModel = new FasSisolahIpalProduksiModel.ViewModel();
                    fieldModel.id = FasSisolahIpalProduksiModel.GetNewId().ToString();
                    ViewData["fieldModel"] = fieldModel;
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = FasSisolahIpalProduksiModel.GetViewData(PkValue);
                    FasSisolahIpalProduksiModel.ViewModel fieldModel = new FasSisolahIpalProduksiModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = FasSisolahIpalProduksiModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(FasSisolahIpalProduksiModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = FasSisolahIpalProduksiModel.GetNewId().ToString();
                    data["spek_ipal_produksi_id"] = fieldModel.spek_ipal_produksi_id;
                    data["sistem_pengolahan_ipal_produksi_id"] = fieldModel.sistem_pengolahan_ipal_produksi_id;
                    ...
                    result = FasSisolahIpalProduksiModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(FasSisolahIpalProduksiModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(FasSisolahIpalProduksiModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(FasSisolahIpalProduksiModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = FasSisolahIpalProduksiModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = FasSisolahIpalProduksiModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = FasSisolahIpalProduksiModel.LookupData(param);
            return Json(data);
        }
