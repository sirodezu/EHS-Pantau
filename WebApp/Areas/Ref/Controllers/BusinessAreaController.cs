...
namespace WebApp.Areas.Ref.Controllers
{
    ...
    public class BusinessAreaController : Controller
    {
        ...
        private string _path_controler = "/Ref/BusinessArea/";
        private string _path_view = "/Areas/Ref/Views/BusinessArea/";
        private readonly string _table_name = "ref_business_area";
        private readonly string _table_title = "Business Area";

		private static IHostingEnvironment _hostingEnvironment;
        public BusinessAreaController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(BusinessAreaModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = BusinessAreaModel._pkKey;
                    ViewData["displayItem"] = BusinessAreaModel.GetDisplayItem();
                    ViewData["column_att"] = BusinessAreaModel.GetGridColumn();
                    ViewData["param"] = BusinessAreaModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(BusinessAreaModel.ActionRequest param)
        {
            GridData data = BusinessAreaModel.GetListData(param);
            return Json(data);
        }
        [HttpGet]
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    BusinessAreaModel.ViewModel fieldModel = new BusinessAreaModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        ...
                        PkValue["id"] = id;
                        data = BusinessAreaModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.area_type_id = dr["area_type_id"].ToString();
                                fieldModel.area_type_id_text = dr["area_type_id_text"].ToString();
        ...
        [HttpGet]
        public IActionResult Add(BusinessAreaModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //BusinessAreaModel.ViewModel fieldModel = new BusinessAreaModel.ViewModel();
                    fieldModel.id = BusinessAreaModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = BusinessAreaModel.GetViewData(PkValue);
                    BusinessAreaModel.ViewModel fieldModel = new BusinessAreaModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = BusinessAreaModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(BusinessAreaModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = BusinessAreaModel.GetNewId().ToString();
                    ...
                    data["area_type_id"] = fieldModel.area_type_id;
                    ...
                    result = BusinessAreaModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(BusinessAreaModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(BusinessAreaModel.GetData(PkValue));
                    ...
                    OrderedDictionary data = DataModel.DtToDictionary(BusinessAreaModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = BusinessAreaModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = BusinessAreaModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = BusinessAreaModel.LookupData(param);
            return Json(data);
        }


        [HttpPost]
        public JsonResult LookupDataBA(lookup_param param)
        {
            DataTable data = BusinessAreaModel.LookupDataBA(HttpContext, param);
            return Json(data);
        }
