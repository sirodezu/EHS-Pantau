...
namespace WebApp.Controllers
{
    ...
    public class CampaignDokumenController : Controller
    {
        ...
        private string _path_controler = "/CampaignDokumen/";
        private string _path_view = "/Views/Campaign/CampaignDokumen/";
        private readonly string _table_name = "ta_cmp_dokumen";
        private readonly string _table_title = "Dokumen Campaign";

		private static IHostingEnvironment _hostingEnvironment;
        public CampaignDokumenController(IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment;
        }

		[HttpGet]
        public IActionResult Index(CampaignDokumenModel.ViewModel filterColumn)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["pkKey"] = CampaignDokumenModel._pkKey;
                    ViewData["displayItem"] = CampaignDokumenModel.GetDisplayItem();
                    ViewData["column_att"] = CampaignDokumenModel.GetGridColumn();
                    ViewData["param"] = CampaignDokumenModel.GetListParam();
        ...
        [HttpPost]
        public JsonResult GetListData(CampaignDokumenModel.ActionRequest param)
        {
            GridData data = CampaignDokumenModel.GetListData(param);
            string baseUrl = WebHelper.GetBaseUrl(HttpContext);
        ...
        public IActionResult ViewItem(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_view))
                {
                    ...
                    ViewData["Title"] = ViewData["TitleHeader"];
                    CampaignDokumenModel.ViewModel fieldModel = new CampaignDokumenModel.ViewModel();
                    ...
                    if (id != null && id !="" )
                    {
                        var PkValue = new OrderedDictionary();
                        PkValue["id"] = id;
                        data = CampaignDokumenModel.GetViewData(PkValue);
                        if (data != null)
                        {
                            foreach (DataRow dr in data.Rows)
                            {
                                ...
                                fieldModel.cmp_id_text = dr["cmp_id_text"].ToString();
        ...
        [HttpGet]
        public IActionResult Add(CampaignDokumenModel.ViewModel fieldModel)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    //CampaignDokumenModel.ViewModel fieldModel = new CampaignDokumenModel.ViewModel();
                    fieldModel.id = CampaignDokumenModel.GetNewId().ToString();
        ...
        public IActionResult Copy(string id)
        {
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id;
                    DataTable data = CampaignDokumenModel.GetViewData(PkValue);
                    CampaignDokumenModel.ViewModel fieldModel = new CampaignDokumenModel.ViewModel();
                    foreach (DataRow dr in data.Rows)
                    {
                        fieldModel.id = CampaignDokumenModel.GetNewId().ToString();
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Insert(CampaignDokumenModel.ViewModel fieldModel)
        {
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_add))
                {
                    ...
                    data["id"] = CampaignDokumenModel.GetNewId().ToString();
                    ...
                    result = CampaignDokumenModel.Insert(_hostingEnvironment, HttpContext, data);
                    return Json(result);
                }
                else
                {
        ...
        [HttpPost]
		[Obsolete]
        public JsonResult Update(CampaignDokumenModel.ViewModel fieldModel)
        {
            ProsesResult result = new ProsesResult();
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_edit))
                {
                    ...
                    var PkValue = new OrderedDictionary();
                    PkValue["id"] = id_old;
                    OrderedDictionary data_old = DataModel.DtToDictionary(CampaignDokumenModel.GetData(PkValue));

                    OrderedDictionary data = DataModel.DtToDictionary(CampaignDokumenModel.GetData(PkValue));
                    ...
                    if (DataModel.HasUpdate(data, data_old))
                    {
                        ...
                        result = CampaignDokumenModel.Update(_hostingEnvironment, HttpContext, data, data_old);
                    }
                    else
                    {
                        result.status = 2;
            ...
            if (SecurityHelper.onPageInit(HttpContext))
            {
                if (SecurityHelper.HasRule(HttpContext, _rule_delete))
                {
                    ...
                    if (jml == 0) {
						...
						if (data != null)
						{
							result = CampaignDokumenModel.Delete(_hostingEnvironment, HttpContext, data);
							return Json(result);
						}
						else
						{
        ...
        {
            DataTable data = CampaignDokumenModel.LookupData(param);
            return Json(data);
        }
